...
cfg_if::cfg_if! {
    if #[cfg(feature = "native")] {
        ...
        fn field_to_array(f: &FieldElement) -> [u8; 32] {
            let v = f.to_be_bytes();
        ...
        impl Barretenberg {
            ...
            fn slice_memory(&self, start: usize, end: usize) -> Vec<u8> {
                let memory = &self.memory;
            ...
            fn call(&self, name: &str, param: &Value) -> WASMValue {
                self.call_multiple(name, vec![param])
            }
            fn call_multiple(&self, name: &str, params: Vec<&Value>) -> WASMValue {
