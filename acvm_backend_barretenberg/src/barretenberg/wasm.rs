...
impl Barretenberg {
    ...
    pub(crate) fn transfer_to_heap(&mut self, arr: &[u8], offset: usize) {
    ...
    pub(crate) fn slice_memory(&self, start: usize, end: usize) -> Vec<u8> {
        let memory = &self.memory;
    ...
    pub(crate) fn call(&self, name: &str, param: &Value) -> WASMValue {
        self.call_multiple(name, vec![param])
    }

    pub(crate) fn call_multiple(&self, name: &str, params: Vec<&Value>) -> WASMValue {
    ...
    pub(crate) fn allocate(&mut self, bytes: &[u8]) -> Value {
        let ptr = self
    ...
    pub(crate) fn free(&mut self, pointer: Value) {
