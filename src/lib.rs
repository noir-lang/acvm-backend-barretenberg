...
mod field_size_checks {
    ...
    fn check_field_to_array() {
        ...
        let _: [u8; FIELD_BYTES] = field_to_array(&field).unwrap();
    }
}
...
mod native {
    ...
    pub(super) fn field_to_array(
        f: &acvm::FieldElement,
    ) -> Result<[u8; FIELD_BYTES], FeatureError> {
        let byte_vec = f.to_be_bytes();
        byte_vec
            .try_into()
            .map_err(|v: Vec<u8>| FeatureError::FieldToArray(FIELD_BYTES, v.len()))
    }
}
